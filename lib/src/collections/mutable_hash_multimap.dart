...
class MutableListMultimap<K,V> extends IterableBase<Pair<K,V>> implements Multimap<K,V> {
  ...
  Sequence<V> operator[](final K key) =>
      _map[key].map((e) => new ForwardingSequence(e)).orElse(PersistentList.EMPTY);

  Dictionary<K, Sequence<V>> asDictionary() => new ForwardingDictionary(_map);

  void put(final K key, final V value) => 
      _map.put(key, 
          _map[key]
            .map((PersistentList<V> list) => list.add(value))
            .orCompute(() =>  PersistentList.EMPTY.add(value)));
...
class MutableSetMultimap<K,V> extends IterableBase<Pair<K,V>> implements Multimap<K,V> {
  ...
  Iterable<V> operator[](final K key) =>
      _map[key].map((e) => new ForwardingSequence(e)).orElse(PersistentHashSet.EMPTY);
  ...
  void put(final K key, final V value) => 
      _map.put(key, 
          _map[key]
            .map((PersistentHashSet<V> set) => set.add(value))
            .orCompute(() =>  PersistentHashSet.EMPTY.add(value)));
